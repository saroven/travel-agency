...
@section('page-subtitle', 'Modify existing client feedback parameters')
...
<div class="w-full">
    ...
    <form method="POST" action="{{ route('admin.testimonials.update', $testimonial) }}" class="space-y-6">
                ...
                <div class="glass-panel rounded-[32px] p-6 md:p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $testimonial->name) }}" required placeholder="e.g. Rafsan Mahmud"
                        ...
                        <div>
                            ...
                            <input type="text" name="company_or_title" value="{{ old('company_or_title', $testimonial->company_or_title) }}" placeholder="e.g. Head of Operations, Apex Ltd"
                                class="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 text-white text-sm placeholder-slate-500 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all" />
                        </div>
                    </div>
                    ...
                    <div class="mt-6">
                        ...
                        <textarea name="quote" rows="5" required placeholder="Paste client feedback here..."
                            class="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 text-white text-sm placeholder-slate-500 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 resize-none transition-all">{{ old('quote', $testimonial->quote) }}</textarea>
                    </div>
                </div>
            </div>
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            ...
                            <select name="stars" class="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-emerald-500 cursor-pointer">
                                @for($i=5; $i>=1; $i--)
                                <option value="{{ $i }}" {{ old('stars', $testimonial->stars) == $i ? 'selected' : '' }}>{{ $i }} Stars</option>
                                @endfor
                            </select>
                        </div>
                        <div class="pt-2">
                            <label class="flex items-center gap-3 cursor-pointer select-none">
                                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $testimonial->is_active) ? 'checked' : '' }} class="rounded border-slate-800 bg-slate-900 text-emerald-500 w-4 h-4 focus:ring-0 focus:ring-offset-0">
        ...
        <div class="flex gap-4 mt-6">
            <button type="submit" class="bg-emerald-500 hover:bg-emerald-600 active:scale-98 text-white font-bold text-sm px-8 py-4 rounded-2xl transition-all shadow-lg shadow-emerald-500/10 cursor-pointer">Update Testimonial</button>
